...
/**
 * Función programada para generar reportes de satisfacción de entrega
 * Se ejecuta los lunes a las 01:00 UTC (reporte semanal)
 * y el primer día de cada mes a las 01:00 UTC (reporte mensual)
 */
export const handler: Handler = async (event, context) => {
  ...
  const reportType = isFirstDayOfMonth ? 'delivery_satisfaction_monthly' : 'delivery_satisfaction_weekly'
  
  console.log(`[${new Date().toISOString()}] Generando reporte ${reportType}...`)
  
  try {
    const response = await fetch(`${appUrl}/api/reports/delivery-satisfaction/generate`, {
      method: 'POST',
      headers: {
