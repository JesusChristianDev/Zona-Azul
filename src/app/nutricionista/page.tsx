    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <section className="grid gap-3 sm:gap-4 md:grid-cols-3">
        {summary.map((item) => (
          <article
            key={item.label}
            className="rounded-2xl border border-primary/20 dark:border-primary/30 bg-white dark:bg-slate-900 p-4 sm:p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-md"
          >
            <p className="text-xs font-semibold uppercase tracking-wider text-primary/60">{item.label}</p>
            <p className="mt-2 sm:mt-3 text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">{item.value}</p>
            <p className="mt-1 sm:mt-2 text-xs font-medium text-gray-500 dark:text-gray-400">{item.delta}</p>
          </article>
        ))}
      </section>
      ...
      <section className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 sm:p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Próximas citas confirmadas</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Citas programadas para los próximos días.</p>
          </div>
          <a
        ...
        <div className="mt-5 space-y-3">
          {upcomingAppointments.length === 0 ? (
            ...
            upcomingAppointments.map((appointment) => (
              <article
                key={appointment.id}
                className="rounded-2xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 transition hover:border-primary/40 dark:hover:border-primary/60 hover:shadow-md animate-in fade-in slide-in-from-bottom-2"
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{appointment.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{appointment.email}</p>
                  </div>
                </div>
