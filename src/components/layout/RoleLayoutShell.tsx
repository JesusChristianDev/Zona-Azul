...
export default function RoleLayoutShell({ title, subtitle, links, children }: RoleLayoutShellProps) {
  ...
  return (
    <div className="min-h-[calc(100vh-160px)] bg-slate-50 py-4 sm:py-6 lg:py-10">
      <div className="mx-auto flex max-w-6xl flex-col gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-6 lg:flex-row">
        <aside className="w-full lg:w-64 space-y-4 sm:space-y-6 flex-shrink-0">
          <div className="rounded-xl sm:rounded-2xl border border-primary/20 bg-white p-4 sm:p-6 shadow-sm animate-in fade-in slide-in-from-top-2">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-primary/70">Zona Azul</p>
            <h1 className="mt-2 text-xl sm:text-2xl font-bold text-gray-900">{title}</h1>
            <p className="mt-2 sm:mt-3 text-xs sm:text-sm text-gray-600">{subtitle}</p>
          </div>
          <nav className="rounded-xl sm:rounded-2xl border border-gray-200 bg-white p-2 shadow-sm">
